...
sheet_name = 'Drinks_March'
df_sales = pd.read_excel('ReporteVentas.xlsx', sheet_name=sheet_name, dtype={'quantity': 'float64'})
sales_data = [tuple(x) for x in df_sales.to_records(index=False)]


# Create a connection to the SQLite database
with sqlite3.connect('March_Drinks_Monthly_Sales.db') as conn:
